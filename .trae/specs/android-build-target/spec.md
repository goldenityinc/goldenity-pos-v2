...
| FR-7 | **App icons launcher Android**: Tambahkan `flutter_launcher_icons` dev dependency. Configure untuk Android: adaptive icon (foreground + background) / PNG fallback untuk legacy. (OQ-2 RESOLVED) Gunakan assets logo asli Andre: `E:\Goldenity\goldenity-pos-v2\assets\logo.png` → copy ke `pos-native-desktop-tablet/assets/images/logo.png` sebelum generate icon. Jalankan build icon via `dart run flutter_launcher_icons`. | Android (icons) | Windows icon `windows/runner/resources/app_icon.ico` TETAP ADA. Tidak perlu generate icon Windows ulang. |
| FR-8 | **Konfigurasi Signing Release Android (OQ-3 RESOLVED)**: (a) Generate release keystore via `keytool -genkeypair` (command exact dari Andre) → output `goldenity-pos-release.jks` di `pos-native-desktop-tablet/android/`. (b) Buat `android/key.properties` berisi nilai actual: `storeFile=goldenity-pos-release.jks`, `storePassword=<pass>`, `keyAlias=goldenity-pos`, `keyPassword=<pass>` (password dipilih Trae, sama store & key). (c) Update `.gitignore` root AND `android/.gitignore` → exclude `key.properties` DAN `*.jks` (TIDAK DI-COMMIT ke repo). (d) Backup file `.jks` + password ke folder luar repo aman: `E:\Goldenity\_keystores\` + simpan password tercatat. (e) Di `app/build.gradle.kts`: tambahkan `signingConfigs.release` yang membaca `android/key.properties` (sama pattern persis V1 reference). (f) Jika keystore / key.properties TIDAK ADA → release build fallback ke signingConfig debug (tidak crash, sesuai V1). (g) Tambahkan instruksi comment untuk command build: `flutter build apk --release --split-per-abi` (menghasilkan 1 APK per ABI: armeabi-v7a, arm64-v8a, x86_64 — sesuai V1 ndk abiFilters). `flutter build appbundle --release` untuk AAB Play Store. Default include ABI: armeabi-v7a + arm64-v8a + x86_64 (32-bit + 64-bit ARM untuk tablet + x86_64 emulator). | Android (signing + build) | Pattern persis V1 reference build.gradle.kts L10-L28 + signingConfigs L76-L106. **Signing untuk sideload APK internal tablet**: key.jks hanya dipakai agar update APK bisa nimpa tanpa uninstall (aman untuk sideload, bukan Play Store production key). |
...
5. ⚠️ **BOLEH generate keystore .jks untuk sideload internal**, TAPI: (a) `*.jks` DAN `key.properties` WAJIB di-exclude dari git via `.gitignore` (root + android/.gitignore) → TIDAK BOLEH ter-commit ke repo. (b) Backup file .jks + password ke folder LUAR repo aman: `E:\Goldenity\_keystores\`.
...
## 8. Open Questions (SEMUA RESOLVED oleh Andre 2026-09-10)

| Q# | Pertanyaan | Jawaban Andre | Status |
|---|---|---|---|
| OQ-1 | Application ID final: `app.goldenity.pos` vs `com.goldenity.pos`? | **`com.goldenity.pos`** (final, untuk sideload + future Play Store). | ✅ **RESOLVED** → FR-1 + AC-R2 diupdate. |
| OQ-2 | Assets launcher icon? Punya file PNG asli? | **Ya:** `E:\Goldenity\goldenity-pos-v2\assets\logo.png` → copy ke `pos-native-desktop-tablet/assets/images/` sebelum generate. | ✅ **RESOLVED** → FR-7 diupdate. |
| OQ-3 | Keystore release: Andre punya `.jks`? Atau Trae generate? | **Trae generate** via keytool command exact dari Andre. Password dipilih Trae, sama store & key. Backup `.jks` + password ke `E:\Goldenity\_keystores\`. | ✅ **RESOLVED** → FR-8 + Hard Constraint 5 diupdate. |
| OQ-4 | **Socket.IO client**: Scope sekarang harus implementasi? Atau polling 6s cukup? | **Polling 6s existing cukup.** Latensi ~6 detik untuk dapur masih wajar. Socket.IO (jaga WS hidup lewat Doze, reconnect) kompleksitas tinggi → jadikan **follow-up task terpisah** jika nanti latensi jadi keluhan. | ✅ **RESOLVED** → Non-Goals 1 dipertahankan. No Socket.IO scope task ini. |
...
**AC-R2: ApplicationId, Label, SDK version sesuai spec (OQ-1 RESOLVED).**
- PASS: `android/app/build.gradle.kts` baris `applicationId = "com.goldenity.pos"`. `defaultConfig` block = `minSdk = 24`, `targetSdk = 34`, `compileSdk = 34` (atau flutter.compileSdkVersion 34+). `AndroidManifest.xml` `<application android:label="Goldenity POS">`.
